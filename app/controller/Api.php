...
class Api extends BaseController
{
    ...
    public function check_domain()
    {
        ...
        if (!$domain)
            return json(['status' => false, 'msg' => '域名不能为空']);
        if (!strpos($domain, '.'))
            return json(['status' => false, 'msg' => '域名格式不正确']);
    ...
    public function update_license()
    {
        ...
        $reqData = hex2bin(input('post.data'));
        parse_str($reqData, $arr);
        if (!isset($arr['product']) || !isset($arr['serverid']))
            return json(['status' => false, 'msg' => '缺少参数']);
    ...
    public function bt_cert()
    {
        ...
        if (!$param || !isset($param['action']) || !isset($param['domain']))
        ...
        if (!$isca)
            return json(['status' => false, 'msg' => 'CA证书不存在']);
        ...
        if ($param['action'] == 'get_domain_cert') {
            ...
            foreach ($domain_list as $d) {
                if (!checkDomain($d))
                    return json(['status' => false, 'msg' => '域名或IP格式不正确:' . $d]);
            }
